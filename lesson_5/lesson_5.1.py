...
for brand, car in car_data.items():
    if (
        car[1] >= SEARCH_CRITERIA[0]
        and car[2] >= SEARCH_CRITERIA[1]
        and car[4] <= SEARCH_CRITERIA[2]
    ):
        cars.append((brand, car))

cars.sort(key=lambda item: item[1][4])
